...
window.mapInit = function() {
    ...
    h.className = "minimap";
    h.innerHTML = html;
    //h.dataset.lastRotate = undefined;

    var c = document.createElement('div');
    html = `<div style="display: none; color: white;text-align: center;position: fixed;left: 173px;right: 0;height: 30px;width: 100px;top: 10px;backdrop-filter: blur(10px);z-index: 21;background: rgba(0,0,0,0.1);border: 1px solid lightgray;display: block;" id="crsTBox"><div style="background: #FF00FF;width: 10%;height: 100%;position: absolute;z-index: 5001;"></div><div style="padding: 5.5px;font-family: 'Roboto';font-size: 16px;" id="crsText">CRS 360-</div></div><div style="display: none; color: white;text-align: center;position: fixed;left: 277px;right: 0;height: 30px;width: 100px;top: 10px;backdrop-filter: blur(10px);z-index: 21;background: rgba(0,0,0,0.1);border: 1px solid lightgray;display: block;" id="adfTBox"><div style="background: #00FF00;width: 10%;height: 100%;position: absolute;z-index: 5001;"></div><div style="padding: 5.5px;font-family: 'Roboto';font-size: 16px;" id="adfText">ADF 360-</div></div>`;
    ...
    <div style="position: fixed;width: 200px;height: 200px;background-image: url(&quot;https://tylerbmusic.github.io/GPWS-files_geofs/crsArrow.svg&quot;);background-repeat: no-repeat;background-size: 100%;top: 0px;rotate: 0deg; display: none; filter: invert(100%); fill: yellow;" id="adfArrow"></div>
...
window.mapUpdate = function() {
    ...
    if (ldgAGL != "N/A") {
        ...
        if (localStorage.getItem("mmHdgUp") == "true") {
            ...
            if (geofs.animation.values.ADFBearing !== undefined) {

                let adfArrow = document.getElementById("adfArrow");

                if (adfArrow) {
                    adfArrow.style.display = "block";

                    let brg =
                        ( //geofs.animation.values.heading +
                            360-
                         geofs.animation.values.ADFBearingToStation) % 360;

                    adfArrow.style.rotate = brg + "deg";
                    document.getElementById("adfTBox").style.display = "block";
                    document.getElementById("adfText").innerHTML = `HDG ${Math.round(geofs.animation.values.ADFBearing) % 360}°`;
            ...
            if (geofs.animation.values.ADFBearing !== undefined) {
                ...
                if (adfArrow) {
                    ...
                    let brg =
                        ( //geofs.animation.values.heading +
                         geofs.animation.values.ADFBearing) % 360;
